...
\hypersetup{
  pdftitle={Why Multi-Domain Decentralized LoRA Falls Short of Pooled Training},
  pdfauthor={AH-LoRA Project Team},
  pdfsubject={A controlled multi-domain LoRA comparison, failure analysis, and validation-only protocol exploration}
}
...
\title{Why Multi-Domain Decentralized LoRA Falls Short of Pooled Training}
...
\section{Related Work and Study Scope}
LoRA represents an adaptation through two trainable low-rank factors while freezing pretrained weights~\citep{hu2022lora}. Our primary testbed applies this parameterization only to a classifier on cached frozen visual features. This controlled setting makes effective-update spectra and optimizer interventions tractable, but does not reproduce full-backbone or language-model fine-tuning. A low factor rank bounds one part of the trainable state; it does not by itself bound probing, aggregation workspace, optimizer cost, or communication control information.

Federated averaging alternates local optimization with aggregation~\citep{mcmahan2017fedavg}. Its sample-weighted target motivates the pooled comparator and the sample-only controls in this study. Local trajectories can nevertheless differ from sequential optimization of the same pooled empirical risk, particularly with heterogeneous label ownership and nonlinear adaptive optimizers. We therefore compare complete executable protocols before attributing a shortfall to either domain weighting or rank adaptation. Separate comparisons under a common rank ceiling test the chosen policies rather than treating a larger, infeasible adapter as a resource-matched reference.

Classical gossip analysis characterizes averaging through graph-supported stochastic matrices~\citep{boyd2006gossip}, and decentralized stochastic optimization studies the interaction of communication and local gradients~\citep{lian2017dpsgd}. These results motivate checking the invariant distribution of the implemented mixing kernel. They do not establish accuracy preservation after changing ranks, truncating effective updates, or applying local Adam steps. Our contribution is a controlled empirical comparison and a set of mechanism-specific diagnostics, with explicit protocol and resource accounting. The operator identities below explain what is preserved by particular steps; they are not new general convergence or privacy guarantees.
...
\end{equation}
A temporary probe at the client's ceiling measures gradient stable rank before local training. For a two-dimensional LoRA factor gradient $G$, define $s(G)=\norm{G}_F^2/\norm{G}_2^2$. The implementation includes only gradients with $\norm{G}_2^2>10^{-12}$ in the median, excluding zero and numerically negligible gradients; if none qualify, the client signal $s_i^t$ is set to 1. The probe uses one fixed, seeded local training minibatch, independently of the training shuffle. No test labels enter the controller.
