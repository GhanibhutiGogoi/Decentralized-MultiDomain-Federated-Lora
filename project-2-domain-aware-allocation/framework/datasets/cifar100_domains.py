...
Adapted from Project 3 with additional helpers for complexity analysis.
"""
...
def create_federated_datasets(
    ...
    Returns:
        clients_data: dict mapping client_id -> {
            ...
            'train_indices': list of dataset indices (for complexity analysis),
        }
    ...
    for domain_id in range(n_domains):
        ...
        for local_id in range(clients_per_domain):
            ...
            clients_data[client_id] = {
                ...
                'train_indices': client_train_splits[local_id],
            }
            client_id += 1

    return clients_data, train_dataset, test_dataset


def get_client_labels(client_data, train_dataset):
    """
    Get numpy array of labels for a specific client.

    Args:
        client_data: single client's dict from clients_data
        train_dataset: the CIFAR-100 training dataset object

    Returns:
        numpy array of integer labels
    """
    all_targets = np.array(train_dataset.targets)
    return all_targets[client_data['train_indices']]
