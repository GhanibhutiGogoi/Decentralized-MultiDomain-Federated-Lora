...
Extended from Project 3 with reset_lora() for oracle rank search.
"""

import torch
import torch.nn as nn
from framework.models.lora_resnet import get_lora_state, set_lora_state, reset_lora_params
...
class FederatedClient:
    ...
    def reset_lora(self):
        """
        Re-initialize LoRA parameters to start fresh.
        Used in oracle rank search to ensure each trial starts from scratch.
        """
        reset_lora_params(self.model)
        self._reset_optimizer()

    def _reset_optimizer(self):
        """Reset optimizer state (needed after parameter changes)."""
        self.optimizer = torch.optim.Adam(
            filter(lambda p: p.requires_grad, self.model.parameters()),
            lr=self.lr,
            weight_decay=1e-4,
