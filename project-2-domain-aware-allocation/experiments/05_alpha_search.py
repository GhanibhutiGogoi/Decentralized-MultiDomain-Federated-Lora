...
Purpose:
    ...
    then search over candidate LoRA alpha values to find the best alpha.

Outputs:
    - results/alpha_search_results.json
    - results/best_alpha_per_client.json
...
def main():
    ...
    if not os.path.exists(oracle_path):
        raise FileNotFoundError(
            "oracle_results.json not found. Run Experiment 02 first."
        )
    ...
    alpha_results_path = os.path.join(results_dir, "alpha_search_results.json")
    with open(alpha_results_path, "w") as f:
        json.dump(alpha_results, f, indent=2)

    best_alpha_path = os.path.join(results_dir, "best_alpha_per_client.json")
    with open(best_alpha_path, "w") as f:
        json.dump(best_alpha_per_client, f, indent=2)

    heatmap_path = os.path.join(results_dir, "alpha_heatmap.png")
    ...
    print("\nSummary:")
    for cid in sorted(best_alpha_per_client.keys(), key=lambda x: int(x)):
        r = best_alpha_per_client[cid]["fixed_rank"]
        a = best_alpha_per_client[cid]["best_alpha"]
        acc = best_alpha_per_client[cid]["best_accuracy"]
        print(f"Client {cid}: rank={r}, best alpha={a}, acc={acc:.4f}")
