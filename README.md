...
The [quantity-skew study](docs/research/2026-09-17-quantity-skew-protocol.md) is complete: seven RoBERTa-base/SST-2 methods across five matched seeds, plus one equal-size anchor, with all 36 full runs and 72 best/final checkpoints verified on gpu003. Adaptive rank with sample-size weighting reaches **94.24 ± 0.25%** best validation accuracy, versus **94.54 ± 0.24%** for an independent Dec-LoRA rank-16 reconstruction and **94.50 ± 0.33%** for rank 4. Adaptive training traffic is **31.81% lower than rank 16**, but **8.92% higher than rank 4**. The study establishes neither an accuracy advantage nor parity; the simpler rank-4 control is cheaper and has higher mean accuracy. [Final evidence and interpretation](docs/artifacts/quantity-skew/README.md) preserve every seed, audit, source snapshot, and resource limitation.

The earlier [methodology exploration](docs/artifacts/methodology-exploration/README.md) is also complete. It isolates optimization and repeated rank-projection losses, preserves failed residual repairs, and shows that a successful full-state gradient-sharing control does not establish the original heterogeneous-rank resource claim. There are now two standalone papers: the [original multi-domain study](paper/multidomain/main.pdf) and the [new quantity-skew comparison](paper/quantity-skew/main.pdf), each with its own methodology, results, figures and conclusions. See the [paper index](paper/README.md).
...
Read the [corrected results](docs/artifacts/integrated-corrected/RESULTS.md), [experiment and provenance guide](docs/artifacts/integrated-corrected/README.md), and [original-study paper](paper/multidomain/main.pdf). The earlier composed pilot is superseded because normalization canceled its weights and its pooled comparison used different scaling. The corrected run uses alpha 32 throughout and repeating resource ceilings 4/8/16. Validation includes 463 remote tests and 3,460 recorded protocol checks.
