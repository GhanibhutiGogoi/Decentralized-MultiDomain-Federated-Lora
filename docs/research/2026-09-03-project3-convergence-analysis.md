...
- **A5 (tail mass).** $\varepsilon^2 := \sup_t \frac{1}{N}\mathbb{E}\|E^t\|_F^2$ is finite. The runner logs its per-round estimate $\frac1N\sum_i\|e_i^t\|_F^2$ (summed over layers) as `mean_residual_energy`; that is the quantity to substitute into the theorems. The runner also logs `mean_tail_mass`, the *relative* form $\|e_i\|^2/\|y_i\|^2$ averaged over clients, which is convenient to read but is not $\varepsilon^2$. $\varepsilon$ is small precisely when the iterates stay close to rank-$r$ matrices — the LoRA hypothesis. Nothing below assumes it is small; the theorems are stated in terms of it.
...
*(a) A product of doubly stochastic matrices is doubly stochastic. (b) If $A, B$ are symmetric then $ABA$ is symmetric. (c) The two-tier matrix $W = W_{\text{intra}} W_{\text{bridge}} W_{\text{intra}}$ of `hierarchical.two_tier_mixing` is symmetric doubly stochastic. (d) [Window reduction — complete intra-cluster mixing only.] Under complete-graph intra-cluster mixing, $W_{\text{intra}}$ is idempotent, so the product over one period of length $L =$ `bridge_every` equals the bridge-round matrix $W$, whatever the window alignment; A3 then holds with that $L$ and $\rho = 1 - |\lambda_2(W)|$. (e) Between bridges the matrix is block-diagonal with one block per cluster, so $1$ is an eigenvalue of multiplicity $K$ and the per-round spectral gap is $0$; the gap lives in the window product. (f) [Bridge-round gap — complete intra-cluster mixing only.] Under the assumption of (d), $W$ is primitive, hence $\rho > 0$. For any other `intra_topology` the window product is still doubly stochastic by (a), but neither (d) nor (f) is claimed: A3's window gap must then be measured, with `window_product` and `spectral_gap`.*

*Proof.* (a) $\mathbf{1}^\top AB = \mathbf{1}^\top B = \mathbf{1}^\top$, $AB\mathbf{1} = A\mathbf{1} = \mathbf{1}$, and entries stay non-negative. (b) $(ABA)^\top = A^\top B^\top A^\top = ABA$. (c) $W_{\text{intra}}$ is block-diagonal with symmetric doubly stochastic Metropolis–Hastings blocks, hence symmetric doubly stochastic; $W_{\text{bridge}}$ embeds a Sinkhorn output -- symmetric exactly, doubly stochastic to the projection tolerance (A3) -- into an identity, hence the same; apply (a) and (b). (d) Metropolis–Hastings on a complete graph $K_m$ gives every weight $1/m$, i.e. the block $J_m$, and $J_m^2 = J_m$; so $W_{\text{intra}}^{j} = W_{\text{intra}}$ for every $j \ge 1$, and any product of one bridge-round matrix with intra-round matrices on either side collapses to $W_{\text{intra}}W_{\text{bridge}}W_{\text{intra}}W_{\text{intra}}\cdots = W$. Singleton and unequal clusters do not affect this ($J_1 = 1$). (e) Each diagonal block has eigenvalue $1$ with its own eigenvector. (f) $W$ has a strictly positive diagonal (each factor does, and products of non-negative matrices with positive diagonals have positive diagonals), and its support is in fact positive across every pair of clients: $J_m$ spreads every member's mass to every member, the representative graph is complete with strictly positive Sinkhorn weights, and the trailing $J_m$ spreads it back. A non-negative matrix with a positive diagonal and connected support is primitive, and Perron–Frobenius gives $|\lambda_2| < 1$. $\blacksquare$
...
*For $W_0$ symmetric doubly stochastic and $w_{\min} \in [0,1)$, $W = w_{\min} I + (1 - w_{\min})W_0$ is symmetric doubly stochastic with $W_{ii} \ge w_{\min}$ and $\lambda_k(W) = w_{\min} + (1-w_{\min})\lambda_k(W_0)$ for every $k$. Consequently the algebraic gap scales exactly, $1 - \lambda_2(W) = (1-w_{\min})(1 - \lambda_2(W_0))$, while the absolute gap that governs consensus satisfies*
$$\rho(W) \;=\; 1 - \max_{k\ge 2}\big|w_{\min} + (1-w_{\min})\lambda_k(W_0)\big| \;\ge\; (1-w_{\min})\,\rho(W_0),$$
*with equality if and only if the non-consensus eigenvalue of $W_0$ of largest magnitude is non-negative.*

*Proof.* Convex combinations of symmetric doubly stochastic matrices are symmetric doubly stochastic; the eigenvalue map is affine because $I$ and $W_0$ share eigenvectors, and it preserves $\lambda_1 = 1$. The algebraic identity is immediate. For the absolute gap, $|w_{\min} + (1-w_{\min})\lambda_k| \le w_{\min} + (1-w_{\min})|\lambda_k|$ by the triangle inequality, with equality iff $\lambda_k \ge 0$; taking the maximum over $k \ge 2$ and subtracting from $1$ gives the bound and its equality condition. $\blacksquare$

So the floor never costs more than the factor $(1-w_{\min})$, and it costs exactly that when $W_0$'s dominant non-consensus mode is non-negative (rings and most affinity kernels). When that mode is negative — a nearly bipartite kernel, where mass oscillates between two sides — the floor damps the oscillation and the gap can *increase*: for $W_0 = \begin{psmallmatrix}0.1&0.9\\0.9&0.1\end{psmallmatrix}$, $\rho(W_0) = 0.2$ but $\rho(W) = 0.9$ at $w_{\min} = 0.5$. An earlier version of this note stated the exact linear scaling unconditionally; that was wrong for exactly this case. Tests: `test_lemma6_floor_shifts_eigenvalues_affinely`, `test_lemma6_absolute_gap_bound_and_exact_case` (both spectra), and `test_self_weight_floor_gap_bound_is_tight_only_for_nonnegative_spectra` in `test_hierarchical.py`.
...
**Established.** Lemmas 1--4, 5(a) and 6 with proofs and numerical pins, with Lemma 3(d)/(f) scoped to complete intra-cluster mixing; Lemma 5(b) as a sketch (the $L^2$ window factor is stated, not worked, and has no numerical pin); two self-contained theorems by the consensus-distance argument; the reduction of the periodic two-tier schedule to a single-matrix analysis under complete-graph intra mixing (Lemma 3d); the exact eigenvalue cost of the self-weight floor (Lemma 6).
