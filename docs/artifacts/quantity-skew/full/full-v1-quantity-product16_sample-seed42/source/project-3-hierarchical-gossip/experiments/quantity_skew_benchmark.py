...
def main():
    ...
    torch.save({"states": states, "ranks": ranks, "capacity": capacity, "weights": weights.tolist(),
                "graph": graph, "config": config}, output / "peers_final.pt")
    summary = {"status": "complete", "arm": args.arm, "seed": args.seed, "partition": args.partition,
               ...
               "peer_checkpoint_sha256": sha256(output / "peers_final.pt"),
