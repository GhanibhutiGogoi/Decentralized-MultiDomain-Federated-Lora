...
Update the existing AH-LoRA artifact from the corrected research evidence in this repository. Read `docs/artifacts/claude_handoff.json`, then `docs/artifacts/integrated-corrected/README.md`, `RESULTS.md`, `summary.json`, `validation.json`, and its standalone paper in `paper/multidomain/` before changing the artifact. The new SST-2 quantity-skew study has a separate paper in `paper/quantity-skew/` and a separate handoff at `docs/artifacts/quantity-skew/CLAUDE_HANDOFF.md`; do not combine their results into one experiment.
