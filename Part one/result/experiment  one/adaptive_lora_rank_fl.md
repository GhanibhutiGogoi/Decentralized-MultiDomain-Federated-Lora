...
This paper investigates the following core question:
...
where:
- $A \in \mathbb{R}^{r \times d}$ — the low-rank down-projection matrix
- $B \in \mathbb{R}^{d \times r}$ — the low-rank up-projection matrix
- $r \ll d$ — the rank, much smaller than the original dimension
...
### 2.3 Heterogeneous Client Setup
...
Global accuracy improves steadily across rounds, but the trajectory is disproportionately shaped by high-capability clients, as their updates carry more weight in the FedAvg step.
...
This creates an **aggregation bias**: clients with more compute steer the global model, regardless of data quality or rank efficiency.

---

### 3.5 Computational Inefficiency
...
**FL performance is resource-driven, not parameter-driven.** Under a uniform rank, the client with the most compute dominates aggregation — not the client with the best-fitted model. The mismatch between model capacity (governed by rank $r$) and computational capability (governed by $E_k$) leads to:

1. **Inefficiency** — wasted computation on saturated low-rank models and wasted parameters on under-trained high-rank models.
2. **Unfairness** — aggregation weights favor resource-rich clients structurally.
3. **Suboptimal convergence** — the global model is shaped by compute budget rather than data relevance.

The core misalignment is between *what the model can represent* and *what the client can compute*.

---

## 5. Conclusion

This paper demonstrates that a **fixed LoRA rank in heterogeneous federated learning** creates a fundamental capacity–computation mismatch with three key consequences:
...
> **Adaptive LoRA rank allocation is necessary for efficient, fair, and scalable federated learning.**

Rank must be treated as a per-client variable, aligned with each client's computational capability rather than held uniform across the federation.
